...
quant_scheme1 = "align_4_3"

def quant1(tensor):
    return getattr(quant,quant_scheme1)(tensor)
		
  

#quantization scheme for middle layers
quant_scheme2 = "align_4_3"
def quant2(tensor):
	return getattr(quant,quant_scheme2)(tensor)
	

#quantization scheme for farther layers
quant_scheme3 = "align_4_3"
def quant3(tensor):
	return getattr(quant,quant_scheme3)(tensor)
...
#-----------------------------Build network encoder based on VGG16 network and load the trained VGG16 weights-----------------------------------------
       #Layer 1
        self.conv1_1 = self.conv_layer8(bgr, "conv1_1") #Build Convolution layer and load weights
        self.conv1_2 = self.conv_layer7(self.conv1_1, "conv1_2")#Build Convolution layer +Relu and load weights
...
##-----------------------Build Net Fully connvolutional layers------------------------------------------------------------------------------------
        ...
        self.conv6 = utils.conv2d_basic(self.pool5 , quant1(W6), quant1(b6))  # Check the size of this net input is it same as input or is it 1X1
        ...
        self.conv7 = utils.conv2d_basic(self.relu_dropout6, quant1(W7), quant1(b7))  # 1X1 Convloution
        ...
        self.conv8 = utils.conv2d_basic(self.relu_dropout7, quant1(W8), quant1(b8))
...
#-------------------------------------Build Decoder --------------------------------------------------------------------------------------------------
        ...
        self.conv_t1 = utils.conv2d_transpose_strided(self.conv8, quant1(W_t1), quant1(b_t1), output_shape=tf.shape(self.pool4))  # Use strided convolution to double layer size (depth is the depth of pool4 for the later element wise addition
        ...
        self.conv_t2 = utils.conv2d_transpose_strided(self.fuse_1, quant1(W_t2), quant1(b_t2), output_shape=tf.shape(self.pool3))
        ...
        self.Prob = utils.conv2d_transpose_strided(self.fuse_2, quant1(W_t3), quant1(b_t3), output_shape=[shape[0], shape[1], shape[2], NUM_CLASSES], stride=8)
...
############################################################################################################################################################
    def conv_layer(self, bottom, name):
        with tf.variable_scope(name):
            ...
            conv = tf.nn.conv2d(bottom, quant1(filt), [1, 1, 1, 1], padding='SAME')

            conv_biases = self.get_bias(name)
            bias = tf.nn.bias_add(conv, quant1(conv_biases))

            relu = tf.nn.relu(bias)
            return relu

    def conv_layer7(self, bottom, name):
        with tf.variable_scope(name):
            filt = self.get_conv_filter(name)

            conv = tf.nn.conv2d(bottom, quant2(filt), [1, 1, 1, 1], padding='SAME')

            conv_biases = self.get_bias(name)
            bias = tf.nn.bias_add(conv, quant2(conv_biases))

            relu = tf.nn.relu(bias)
            return relu

    def conv_layer8(self, bottom, name):
        with tf.variable_scope(name):
            filt = self.get_conv_filter(name)

            conv = tf.nn.conv2d(bottom, quant3(filt), [1, 1, 1, 1], padding='SAME')

            conv_biases = self.get_bias(name)
            bias = tf.nn.bias_add(conv, quant3(conv_biases))
