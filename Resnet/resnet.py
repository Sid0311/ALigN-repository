...
class ResNet(object):
    ...
    def build_tower(self, images, labels):
        ...
        print("--------------------------------------------------------------------------------------------------------------------------------------",labels, preds)
        acc_top1 = tf.reduce_mean(correct)
        acc_top5 = tf.reduce_mean(tf.cast(tf.nn.in_top_k(predictions=logits, targets=labels, k=5), tf.float32))

        # Loss & acc
        losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=x, labels=labels)
        loss = tf.reduce_mean(losses)

        return logits, preds, loss, acc_top5
