...
#quantization scheme 
quant_scheme = "align_4_3"
def quant1(tensor):
    return getattr(quant,quant_scheme)(tensor)
...
def _conv(x, filter_size, out_channel, strides, pad='SAME', input_q=None, output_q=None, name='conv'):
    ...
    with tf.variable_scope(name):
        ...
        conv = tf.nn.conv2d(x, quant1(kernel), [1, strides, strides, 1], pad)
...
def _fc(x, out_dim, input_q=None, output_q=None, name='fc'):
    ...
    with tf.variable_scope(name):
        ...
        fc = tf.nn.bias_add(tf.matmul(x, quant1(w)), quant1(b))
